# Ctrip case
# 此case用于测试选择每个供应商的一条航线去进行验价，检查座位数取严，开票航司，价格是否一致；
...
class Case_Ctrip_Verify_0002(CtripCase):

    def __init__(self):
        CtripCase.__init__(self)
        self.log.info("===Case_Ctrip_Verify_0002,测试开始===")
    ...
    def TestResult(self):
        self.log.info('===Case_Ctrip_Verify_0002,测试完毕===')
        if self.flag:
            self.log.info('=========Case_Ctrip_Verify_0002,测试通过')
            print("测试结果很成功，perfect！")
        else:
            self.log.error('=========Case_Ctrip_Verify_0002,测试失败')
