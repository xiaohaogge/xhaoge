...
class Case_Currency_OAutoRise_0103(CaseBase):
    ...
    def TestProcess(self):
        ...
        # todo 暂不实现；
        reqC = {"pro":"sscts","cid":"ctrip","from":"USD","to":"CNY"}
    ...
    def TestResult(self):
        ...
        if self.flag:
            ...
            self.log.info('=========Case_Currency_OAutoRise_0103,测试失败')
    ...
    def getCountAfter(self,nowC,oriC,percentage):
        '''定义计算方式，result=（现市场汇率-原指定汇率）*升汇百分比+原指定汇率'''
        if type(nowC)!=float and type(oriC)!=float and type(percentage)!=float:
            return '参数有误，请检查！'
