# masterCurrency case
# 此case用于测试rule加价的准确与否，含退改加价到最后的报价rule(无复用)；
...
class Case_Search_KeyValue_0005(SearchBase):

    def __init__(self):
        SearchBase.__init__(self)
        self.log.info("===Case_Search_KeyValue_0005,测试开始===")


    def TestProcess(self):
        self.log.info('===Case_Search_KeyValue_0005,进入测试步骤！===')
    ...
    def TestResult(self):
        self.log.info('===Case_Search_KeyValue_0005,测试完毕===')
        if self.flag:
            self.log.info('Case_Search_KeyValue_0005,测试通过')
            print("测试结果很成功，perfect！")
        else:
            self.log.info('【Case_Search_KeyValue_0005,测试失败】')
