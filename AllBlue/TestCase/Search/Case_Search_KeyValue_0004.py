# masterCurrency case
# 此case用于测试rule加价的准确与否，只有原始退改最后的报价rule(无复用和退改加价)；
...
class Case_Search_KeyValue_0004(SearchBase):

    def __init__(self):
        SearchBase.__init__(self)
        self.log.info("===Case_Search_KeyValue_0004,测试开始===")


    def TestProcess(self):
        self.log.info('===Case_Search_KeyValue_0004,进入测试步骤！===')
    ...
    def TestResult(self):
        self.log.info('===Case_Search_KeyValue_0004,测试完毕===')
        if self.flag:
            self.log.info('Case_Search_KeyValue_0004,测试通过')
            print("测试结果很成功，perfect！")
        else:
            self.log.info('【Case_Search_KeyValue_0004,测试失败】')
